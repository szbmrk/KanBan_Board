...
export default function Permissiontable() {
    const minNewRoleNameLength = 1;
    const maxNewRoleNameLength = 255;
    ...
    async function handleRoleRenameSubmit(role_id) {
        const currentRole = roles.find(
            (role) => parseInt(role.role_id) === parseInt(role_id)
        );
        ...
        <div className="content col-10" >
            ...
            {checkPermissionForBoard(board_id, team_id, "role_management") && (
                <div className="add-role">
                    <input
                        type="text"
                        maxLength={maxNewRoleNameLength}
                    ...
                    <button
                        onClick={AddNewRole}
                        disabled={newRoleName.length < minNewRoleNameLength}
