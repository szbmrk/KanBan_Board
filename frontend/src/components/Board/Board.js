...
const Board = () => {
    ...
    const [inspectedCodeReviewOrDocumentation, setInspectedCodeReviewOrDocumentation] = useState(null);
    ...
    const deleteCodeReviewOrDocumentation = async (codeReviewOrDocumentationToDelete) => {
        ...
        if (deleteCodeReviewOrDocumentation) {
            const filteredCodeReviewOrDocumentations = codeReviewOrDocumentations.filter(
                (codeReviewOrDocumentation) =>
                    codeReviewOrDocumentation.agi_answer_id !== codeReviewOrDocumentationToDelete.agi_answer_id
            );
    ...
    const getCraftedPromptResult = async (craftedPrompt) => {
        ...
        return await axios.get(`/boards/${board_id}/AGI/crafted-prompts/${craftedPrompt.crafted_prompt_id}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
                        ...
                        <div className='agi-submenu' onMouseLeave={() => setIsAGIOpen(false)}>
                            ...
                            <ul className='agi-menu'>
                                ...
                                {codeReviewOrDocumentations.map((element, index) => (
                                    <ul className='code-review-or-documentation-container' key={index}>
                                        <li className='code-review-or-documentation-title'>
                                            ...
                                            >
                                                <span
                                                    className='code-button'
                                                    style={{
                                                        color: isHoveredCode === index + 1 ? 'var(--code)' : '',
                                                    }}
                                                >
                                                    {codeIcon}
                                                </span>
