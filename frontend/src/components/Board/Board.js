...
const Board = () => {
  ...
  const handleColumnNameConfirm = async (data) => {
    ...
    try {
      ...
      newColumn.is_finished = data.isFinished ? 1 : 0;
      newColumn.task_limit = data.taskLimit;
      newColumn.tasks = [];
  ...
  const handleDeleteButtonClick = (event, columnIndex) => {
    console.log("delete columnIndex");
  ...
  const handleColumnDeleteConfirm = () => {
    ...
    try {
      axios.delete(
        `/boards/${board_id}/columns/${board.columns[columnIndex].column_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const newBoardData = [...board.columns];
      newBoardData.splice(columnIndex, 1);
  ...
  const handleDotsClick = (event, columnIndex) => {
    ...
    console.log("localColumnIndex");

    handleIconContainerOptions();
    console.log("triple dot columnIndex");
    console.log(columnIndex);
    //Itt nem update-el
  ...
  const options = [
    {
      ...
      onClick: () =>
        openGenerateTaskWithAGIPopup(null, board.columns[columnIndex]),
    ...
    })),
    {
      ...
      onClick: (e) => {
        console.log("button columnIndex");
        console.log(columnIndex);
        handleDeleteButtonClick(e, columnIndex);
      },
      ...
      {permission === false ? (
        ...
        <DndProvider backend={HTML5Backend}>
                                ...
                                {craftedPromptsBoard.map((craftedPrompt, index) => (
                                    ...
                                    >
                                        <span
                                            ...
                                            style={{
                                                color: isHoveredClipboard === index + 1 ? 'var(--light-blue)' : '',
                                            }}
                                        >
                                            {clipboardIcon}
                                        </span>
          ...
          {showDeleteConfirmation && (
            <ConfirmationPopup
              text={board.columns[columnIndex]?.name}
